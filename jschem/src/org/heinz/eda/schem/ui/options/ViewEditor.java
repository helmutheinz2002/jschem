...
public class ViewEditor extends StandardDialogPanel {
	...
	private int[] DEFAULT_GRIDS = new int [] {
			10, 20, 25, 50,
			100, 200, 250, 500,
			1000, 2000, 2500, 5000
		};
	...
	public ViewEditor() {
		...
		for(int i=0; i<SheetSize.SIZES.length; i++)
			pageSizes.addItem(SheetSize.SIZES[i]);
	...
	public void ok() {
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SNAP, new Boolean(snapGridButton.isSelected()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SNAP_SPACING, new Integer(snapGridSizes.getSelectedUnits()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_VISIBLE, new Boolean(showGridButton.isSelected()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SPACING, new Integer(gridSizes.getSelectedUnits()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_DEFAULT_SHEET_SIZE, new Integer(((SheetSize) (pageSizes.getSelectedItem())).key));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_ANTIALIASING, new Boolean(antiAliasingButton.isSelected()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_SMART_JUNCTIONS, new Boolean(smartJunctionsButton.isSelected()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_SMART_JUNCTIONS_OUTLINE, new Boolean(smartJunctionsOutlineButton.isSelected()));
		SchemOptions.instance().setOption(SchemOptions.PROPERTY_SELECTION_CONTAINS, new Boolean(selectionContains.isSelected()));
	}
}
