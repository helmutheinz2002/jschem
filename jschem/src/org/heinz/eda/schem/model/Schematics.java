...
public class Schematics {
		...
		propertyChangeListener = new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if(evt.getPropertyName().equals(Sheet.PROPERTY_DIRTY)) {
					...
					pcs.firePropertyChange(PROPERTY_DIRTY, (Boolean) new Boolean(oldVal), (Boolean) new Boolean(isDirty()));
				}
			}
		};
	}
	...
	public int guessSnapGrid(List onGridPins, List offGridPins) {
		...
		for(Iterator it=sheets.iterator(); it.hasNext();) {
			Sheet s = (Sheet) it.next();
			s.collectPins(s.getComponents(), pinCoords);
		}
	...
	public Sheet addSheet(String title, boolean init) {
	...
	private void setMeDirty(boolean dirty) {
		...
		this.dirty = dirty;
		boolean d = isDirty();
		pcs.firePropertyChange(PROPERTY_DIRTY, (Boolean) new Boolean(oldVal), (Boolean) new Boolean(d));
	}
