...
public class SnapGridHelper {
	...
	public static void adjustSnapGrid(Schematics schematics, Component optionPaneParent) {
		...
		if(guessGrid < 5) {
			if(gridSnap) {
				...
				if(res == JOptionPane.YES_OPTION) {
					SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SNAP, false);
				}
			}
			return;
		}
		...
		if(!gridSnap || (gridSnap && ((guessGrid % currentGrid) != 0))) {
			...
			if(res == JOptionPane.YES_OPTION) {
				SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SNAP, true);
				SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SNAP_SPACING, guessGrid);
				SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_VISIBLE, true);
				int visibleGrid = GridHelper.guessVisibleGrid(guessGrid);
				SchemOptions.instance().setOption(SchemOptions.PROPERTY_GRID_SPACING, visibleGrid);
