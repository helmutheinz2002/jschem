...
public class ExportHelper {
	...
	public static Map getDefaultProperties(String author) {
		...
		props.put(ExportHelper.PROPERTY_SHOW_FOOTER_PAGE_NR, true);
		return props;
	}
	...
	public static JFileChooser getFileChooser(String dirName, String lastExtension, ExportFormat[] formats) {
		...
		for(CustomFileFilter ff : filters) {
			ExportFormat ef = (ExportFormat) (ff.getFormatObject());
	...
	private static File findTempFile(String extension) {
		...
		for(int i = 0;; i++) {
			String fn = tempDir + File.separator + "tmpfile" + i + extension;
			File f = new File(fn);
			if(!f.exists()) {
				return f;
			}
